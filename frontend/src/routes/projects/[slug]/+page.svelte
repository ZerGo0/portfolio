    ...
    <div class="flex flex-col items-center overflow-x-hidden">
      <Banner img={getAssetURL(data.project.logo)}>
        <div class="col-center p-y-20">
          ...
          <div class="row-center flex-wrap text-[0.9em] text-[var(--tertiary-text)] m-b-2">
            {#each data.project.links as item (item.label)}
              <Chip href={item.to}>
      ...
      <div class="pt-3 pb-1 overflow-x-hidden w-full">
        ...
        {#if screenshots.length > 0}
          ...
          >
            {#each screenshots as item, index (item.src)}
              <button
                class="col-center gap-3 overflow-hidden w-100% h-100% rounded-10px bg-transparent border-none p-0 cursor-pointer"
